...
public class ECommerceApp extends HttpApp {
	...
	public static void main(String[] args) throws Exception {
		URL filePath = ECommerceApp.class.getClassLoader().getResource("front.conf");
		Config config = ConfigFactory.parseFile(new File(filePath.toURI()));
		frontSystem = ActorSystem.create("front", config);
		
		
		ActorRef recommendations = frontSystem.actorOf(FromConfig.getInstance().props(), "recommendations");
		ActorRef trend = frontSystem.actorOf(FromConfig.getInstance().props(), "trends");
		
		ActorRef frontActor = frontSystem.actorOf(FromConfig.getInstance().props(FrontActor.getProps(recommendations, trend)), 
				 "frontActor");
		greeterActor = frontSystem.actorOf(FromConfig.getInstance().props(GreeterActor.getProps()), 
				 "greeterActor");
